...
async function groupRoutes(app) {
    ...
    app.post('/groups', { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        const code = await (0, generateGroupCode_1.generateGroupCode)(app.prisma);
    ...
    app.get('/groups/search', { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        if (!code?.trim())
            return reply.status(400).send({ message: 'Informe o código do grupo.' });
    ...
    app.post('/groups/:groupId/join', { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        if (existing)
            return reply.status(409).send({ message: 'Você já é membro deste grupo.' });
    ...
    app.delete('/groups/:groupId/leave', { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        if (!membership)
            return reply.status(404).send({ message: 'Você não é membro deste grupo.' });
        if (membership.role === 'OWNER')
            return reply.status(400).send({ message: 'O criador não pode sair do grupo. Delete o grupo.' });
    ...
    app.post('/groups/:groupId/challenges', { preHandler: [app.authenticate] }, async (req, reply) => {
        const userId = req.user.sub;
        const { groupId } = req.params;
        const { title, description, goal, startDate, endDate, type, weeklyCheckinLimit, minSessionMinutes, maxSessionMinutes, scoreStrength, scoreCardio, scoreSports, } = req.body;
        const group = await req.server.prisma.group.findUnique({ where: { id: groupId } });
        if (!group || group.creatorId !== userId)
        ...
        if (!goal || goal < 1)
            return reply.status(400).send({ message: 'Meta deve ser ao menos 1.' });
        if (!startDate || !endDate)
            return reply.status(400).send({ message: 'Datas são obrigatórias.' });
    ...
    app.get('/groups/:groupId/challenges/:challengeId/ranking', { preHandler: [app.authenticate] }, async (req, reply) => {
        ...
        const ranking = members
            ...
            let score = 0;
            if (challenge.type === 'CHECKIN_COUNT')
                score = userCheckins.length;
            else if (challenge.type === 'SCORE')
                score = userCheckins.reduce((s, c) => s + c.points, 0);
            else if (challenge.type === 'TOTAL_TIME')
                score = userCheckins.reduce((s, c) => s + c.minutes, 0);
            return {
                user: m.user,
    ...
    app.post('/groups/:groupId/challenges/:challengeId/checkin', { preHandler: [app.authenticate] }, async (req, reply) => {
        const userId = req.user.sub;
        const { groupId, challengeId } = req.params;
        const { sessionId, note, workoutType } = req.body;
        const membership = await req.server.prisma.groupMember.findUnique({
            where: { groupId_userId: { groupId, userId } },
        });
        if (!membership)
        ...
        if (now < challenge.startDate)
            return reply.status(400).send({ message: 'O desafio ainda não começou.' });
        if (now > challenge.endDate)
            return reply.status(400).send({ message: 'O desafio já encerrou.' });
        ...
        if (sessionId) {
            ...
            if (session?.duration)
                sessionMinutes = Math.floor(session.duration / 60);
        }
        ...
        if (challenge.weeklyCheckinLimit) {
            ...
            if (weekCheckins >= challenge.weeklyCheckinLimit)
                countedInWeeklyLimit = false;
        }
        // ─── Calcula pontos (só se tipo SCORE e dentro do limite)
        let points = 0;
        if (challenge.type === 'SCORE' && countedInWeeklyLimit) {
            if (workoutType === 'strength')
                points = challenge.scoreStrength;
            else if (workoutType === 'cardio')
                points = challenge.scoreCardio;
            else if (workoutType === 'sports')
                points = challenge.scoreSports;
        }
        ...
        if (challenge.type === 'CHECKIN_COUNT')
            ...
            total = userCheckins.reduce((s, c) => s + c.points, 0);
        else if (challenge.type === 'TOTAL_TIME')
            total = userCheckins.reduce((s, c) => s + c.minutes, 0);
