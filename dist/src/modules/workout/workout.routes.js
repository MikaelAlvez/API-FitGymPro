...
const exerciseSchema = zod_1.z.object({
    ...
    sets: zod_1.z.string().default(''),
    reps: zod_1.z.string().default(''),
    order: zod_1.z.number().int().default(0),
    type: zod_1.z.enum(['exercise', 'cardio']).default('exercise'),
    groupId: zod_1.z.string().optional(),
    groupLabel: zod_1.z.string().optional(),
    duration: zod_1.z.string().optional(),
    load: zod_1.z.string().optional(),
    restTime: zod_1.z.string().optional(),
    isDrop: zod_1.z.boolean().optional(),
    dropSets: zod_1.z.string().optional(),
});
const createWorkoutSchema = zod_1.z.object({
    studentId: zod_1.z.string().uuid(),
    name: zod_1.z.string().min(1, 'Nome do treino obrigatório'),
    days: zod_1.z.array(zod_1.z.string()).min(1, 'Selecione ao menos um dia'),
    notes: zod_1.z.string().max(500).optional(),
    exercises: zod_1.z.array(exerciseSchema).min(1, 'Adicione ao menos um exercício'),
});
const updateWorkoutSchema = zod_1.z.object({
    name: zod_1.z.string().min(1).optional(),
    days: zod_1.z.array(zod_1.z.string()).min(1).optional(),
    notes: zod_1.z.string().max(500).optional(),
    exercises: zod_1.z.array(exerciseSchema).optional(),
});
const mapExercisesCreate = (exercises) => exercises.map((e, i) => ({
    name: e.name,
    sets: e.sets,
    reps: e.reps,
    order: e.order ?? i,
...
const mapExercisesCreateMany = (exercises, workoutId) => exercises.map((e, i) => ({
    workoutId,
    name: e.name,
    sets: e.sets,
    reps: e.reps,
...
async function listWorkoutsController(req, reply) {
...
// ─── POST /workouts ───────────────────────────
async function createWorkoutController(req, reply) {
...
// ─── PUT /workouts/:id ────────────────────────
async function updateWorkoutController(req, reply) {
    const { id } = req.params;
    const userId = req.user.sub;
    const role = req.user.role;
...
// ─── DELETE /workouts/:id ─────────────────────
async function deleteWorkoutController(req, reply) {
    const { id } = req.params;
    const userId = req.user.sub;
    const role = req.user.role;
    const existing = await req.server.prisma.workout.findUnique({ where: { id } });
...
// ─── GET /workouts/my ─────────────────────────
async function myWorkoutsController(req, reply) {
...
async function deactivateWorkoutController(req, reply) {
    const { id } = req.params;
    const userId = req.user.sub;
    const role = req.user.role;
    const existing = await req.server.prisma.workout.findUnique({ where: { id } });
...
async function activateWorkoutController(req, reply) {
    const { id } = req.params;
    const userId = req.user.sub;
    const role = req.user.role;
    const existing = await req.server.prisma.workout.findUnique({ where: { id } });
...
// ─── Register routes ──────────────────────────
async function workoutRoutes(app) {
