...
export async function friendRoutes(app: FastifyInstance) {
  ...
  app.post('/friends/request/:userId', { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    if (existing) {
      if (existing.status === 'ACCEPTED') return reply.status(409).send({ message: 'Vocês já são amigos.' })
      if (existing.status === 'PENDING')  return reply.status(409).send({ message: 'Solicitação já enviada.' })
  ...
  app.post('/sessions/:sessionId/comments', { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    if (!text?.trim()) return reply.status(400).send({ message: 'Comentário não pode ser vazio.' })
